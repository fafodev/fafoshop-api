...
CREATE TABLE product (
  ...
  cost                  DECIMAL(12,2)  NULL COMMENT 'Giá vốn hiện hành của đơn vị lẻ - cấu hình/cập nhật trực tiếp trong Product Master (Nhập hàng đổi giá thì hỏi xác nhận rồi ghi đè cột này), KHÔNG tính bình quân gia quyền từ inbound_receipt_item. NULL = chưa từng cấu hình/chưa nhập hàng lần nào.',
...
CREATE TABLE inbound_receipt_item (
  ...
  unit_name        VARCHAR(20)   NULL COMMENT 'Tên đơn vị đóng gói đã chọn lúc nhập (vd "Lốc") — NULL = đơn vị lẻ. CHỈ phục vụ hiển thị lại, KHÔNG dùng tính toán (actual_qty đã quy đổi ra lẻ mới là nguồn sự thật).',
  unit_qty         INT(9)        NULL COMMENT 'Số lượng theo ĐÚNG đơn vị đã chọn (vd 3, nếu unit_name="Lốc") — NULL khi unit_name NULL.',
  line_amount      DECIMAL(12,2) NULL COMMENT 'Thành tiền CHÍNH XÁC của dòng (làm tròn ĐÚNG 1 lần khi thêm qua đơn vị đóng gói không chia hết) — NULL = tính bình thường unit_cost × actual_qty, mirror sale_order_item.line_amount.',
  price            DECIMAL(12,2) NULL COMMENT 'Giá bán ĐÃ ÁP DỤNG lúc lập phiếu này (đồng thời ghi vào product.price) — lưu lại để xem lại đúng ở Chi tiết phiếu nhập.',
  note             VARCHAR(200)  NULL COMMENT 'Ghi chú',
  entry_user_code  VARCHAR(8)    NOT NULL COMMENT 'Mã người dùng tạo bản ghi',
  entry_datetime   TIMESTAMP(6)  NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT 'Thời điểm tạo bản ghi',
  entry_program    VARCHAR(10)   NOT NULL COMMENT 'Mã chương trình tạo bản ghi',
...
CREATE TABLE sale_order_item (
  ...
  unit_name        VARCHAR(20)   NULL COMMENT 'Tên đơn vị đóng gói đã chọn lúc bán (vd "Lốc") — NULL = đơn vị lẻ. CHỈ phục vụ hiển thị lại, KHÔNG dùng tính toán (quantity đã quy đổi ra lẻ mới là nguồn sự thật).',
  unit_qty         INT(9)        NULL COMMENT 'Số lượng theo ĐÚNG đơn vị đã chọn (vd 3, nếu unit_name="Lốc") — NULL khi unit_name NULL.',
  entry_user_code  VARCHAR(8)    NOT NULL COMMENT 'Mã người dùng tạo bản ghi',
  entry_datetime   TIMESTAMP(6)  NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT 'Thời điểm tạo bản ghi',
  entry_program    VARCHAR(10)   NOT NULL COMMENT 'Mã chương trình tạo bản ghi',
