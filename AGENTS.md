# AGENTS.md — Hướng Dẫn Cho Cursor (`fafoshop-api`, backend Java)

> Cursor tự động đọc file này khi làm việc trong folder `fafoshop-api/`. Đây
> là **bản tương đương AGENTS.md** của `.claude/CLAUDE.md` trong cùng
> project (nội dung giữ đồng bộ về ý nghĩa — `.claude/CLAUDE.md` vẫn còn
> nguyên, dùng cho Claude Code nếu cần). Xem thêm `../AGENTS.md` (gốc
> workspace) cho ngữ cảnh cross-project.

Đây là dự án Java (Jersey JAX-RS + JDBC thuần, không ORM) làm API cho hệ thống
bán lẻ/tạp hoá `fafoshop` (frontend Angular ở `../fafoshop`, cùng workspace
này).
...
Controller/Service/DAO thông thường — xem chi tiết trong `.claude/architecture.md`.
Xem `.claude/retail-domain.md` để biết vai trò từng bảng nghiệp vụ và những
quy tắc còn `UNKNOWN`.
...
- **[ƯU TIÊN CAO NHẤT] Bảo mật không được thoả hiệp.** TUYỆT ĐỐI KHÔNG viết
  ...
  - Không hardcode secret/credential DB vào code commit lên repo (đã có cơ
    chế `db.properties` + `AES128AndBase64` cho việc này — LƯU Ý: cơ chế mã
    hoá này có lỗ hổng đã biết, xem `docs/pos-deploy-production.md` mục 2 #2
    ở workspace gốc — khoá giải mã hardcode ngay trong source cùng repo,
    KHÔNG bảo vệ được gì thật sự nếu ai đó đọc được repo).
...
- Toàn bộ nội dung tạo trong repository (comment, Javadoc, tài liệu,
...
- Không phát minh API/nghiệp vụ mà `.claude/retail-domain.md` đánh dấu `UNKNOWN`.
...
Các file trên GIỮ NGUYÊN từ bản gốc (workspace `fafoshop-workspace`) — đọc
đúng như Claude Code vẫn làm, Cursor chỉ khác ở điểm vào (`AGENTS.md` này
thay cho việc tự nhớ đọc `.claude/CLAUDE.md`).
...
## Lỗi Encoding (Mojibake) — Kinh Nghiệm Thực Tế, hay bị mắc
...
Khi viết/chạy 1 chương trình Java tạm (`javac`/`java`) trên máy dev Windows
để thao tác dữ liệu tiếng Việt (đọc/ghi DB qua JDBC, in ra để review...),
PHẢI làm ĐỦ 3 việc, thiếu 1 là dữ liệu/hiển thị sai:
...
3. **In ra console để review**: `java -Dstdout.encoding=UTF-8
   ...
   nên chữ có dấu in ra terminal sẽ thành `?`/ký tự vỡ dù dữ liệu Java/DB bên
   ...
   ghi output ra file rồi đọc file đó bằng tool đọc UTF-8 đúng (vd Read của
   Cursor) trước khi kết luận.
...
## Khác biệt DB so với workspace gốc — không có gì thay đổi

Kết nối tới CÙNG MySQL 8.4 `fafoshop_pos` (`localhost:3307`) như workspace
gốc (workspace này chỉ copy source code, KHÔNG copy/nhân bản database) — sửa
DB ở đây (qua Process/migration) sẽ ảnh hưởng CÙNG 1 database mà workspace
gốc `fafoshop-workspace/fafoshop-api` cũng đang trỏ tới. Cân nhắc trước khi
chạy migration/seed nếu định dùng song song cả 2 workspace.
